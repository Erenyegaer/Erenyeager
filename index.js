...
const instagram = 'http://www.instagram.com/regal_uchiha27'; 
const vcard = 'BEGIN:VCARD\n'
            + 'VERSION:3.0\n' 
            + 'FN:mr.pler\n' // Nama kamu
            + 'ORG:eren;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=6285780665726' //Nomor whatsapp kamu
            + 'END:VCARD'
const
{
WAConnection,
...
conn.on('user-presence-update', json => console.log(json.id + ' presence is => ' + json.type)) || console.log('EREN BOT')
conn.on('message-status-update', json =>
{
   const participant = json.participant ? ' (' + json.participant + ')' : ''
   console.log(`[ ${moment().format("HH:mm:ss")} ] => bot by ig:@eren bot`)
})
...
conn.sendMessage(id, 'Halo senpai\nSelamat bergabung\n\n╭════•›「 *INTROD* 」\n│ Nama    :\n│ Umur     :\n│ Status   :\n│ Gender  :\n│ Askot    :\n╰═══════════════',MessageType.text);
}
  //Tag
...
if (text.includes('#tagme')) {
 ...
 const options = {
       text: `@${nomor.split("@s.whatsapp.net")[0]} Alo kak 🥰`,
...
conn.sendMessage(id, 'Silakan ulangi command dengan huruf kecil\n_contoh : #nulis mas eren itu keren_',MessageType.text, { quoted: m } );
}
if (text.includes('#nulis')){
      ...
      imageToBase64(res.data.result)
        .then(
          (ress) => {
            conn.sendMessage(id, '[ WAIT ] Menulis ⏳ silahkan tunggu', MessageType.text, {quoted: m})
            var buf = Buffer.from(ress, 'base64')
            conn.sendMessage(id, buf ,MessageType.image, {caption: 'Nulis gini aja males lu cok', quoted: m})
        })
    })
}
  //Pengucapan ulang
if (text.includes('#Say')){
conn.sendMessage(id, 'Silakan ulangi command dengan huruf kecil\n_contoh : .say eren gans_',MessageType.text, {quoted: m});
}
if (text.includes("#say")){
...
conn.sendMessage(id, 'Silakan ulangi dengan mengirim foto dengan caption , itu aja gatau gblk.ocr',MessageType.text, {quoted: m});
}
...
conn.sendMessage(id, 'Silakan ulangi dengan mengirim foto dengan caption .stiker , gitu aja harus dikasi tau ajg',MessageType.text, {quoted: m});
}
   if (messageType == 'imageMessage')
   {
      let caption = imageMessage.caption.toLocaleLowerCase()
...
if (text.includes(".quotes"))
   {
    ...
    conn.sendMessage(id, '[ WAIT ] Searching ⏳ silahkan tunggu', MessageType.text, { quoted: m } )
	var buf = Buffer.from(response, 'base64'); 
              conn.sendMessage(id, buf ,MessageType.image, { caption: `Nih cok`, quoted: m } )
            }
        )
        .catch(
            (error) => {
...
conn.sendMessage(id, 'Silakan ulangi command dengan huruf kecil\n_contoh : .namae eren_',MessageType.text, { quoted: m } );
}
if (text.includes("#namae")){
...
conn.sendMessage(id, 'Silakan ulangi command dengan huruf kecil\n_contoh : #logopornhub eren/nime_',MessageType.text, { quoted: m } );
}
if (text.includes('#logopornhub')){
...
conn.sendMessage(id, 'Silakan ulangi command dengan huruf kecil\n_contoh : .lion eren/nime_',MessageType.text, { quoted: m } );
}
if (text.includes('#lion')){
...
conn.sendMessage(id, 'Silakan ulangi command dengan huruf kecil\n_contoh : .joker eren_',MessageType.text, { quoted: m } );
...
conn.sendMessage(id, 'Silakan ulangi command dengan huruf kecil\n_contoh : .Snow eren_',MessageType.text, { quoted: m } );
}
if (text.includes('.snow')){
...
//Random Al-Qur'an
if (text.includes('#al-quran')){
conn.sendMessage(id, 'Silakan ulangi command dengan huruf kecil',MessageType.text, { quoted: m } );
}
else if (text == '#al-quran'){
...
conn.sendMessage(id, 'Silakan ulangi command dengan huruf kecil\n_contoh : .couple eren & mikasa_',MessageType.text, { quoted: m } );
}
if (text.includes('#couple')){
...
͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏͏
MOHON MAAF JIKA ADA FITUR YANG ERROR 
...
├≽️ *#Getzodiak*
├ _Ex : #Getzodiak udin & 09-09-2009_ 
├≽️ *#Zodiak (zodiak)*
├ _Ex : #Zodiak libra_ 
├≽️ *#Mimpi (teks)*
├ _Ex : #Mimpi titan_
...
├ _Ex = #Kata Aku cinta dia/Eren_ 
├≽️ *#Logopornhub (teks1/teks2)*
├ _Ex : #Logopornhub Eren/nime 
├≽️ *#Lion (teks1/teks2)*
├ _Ex : #Lion Eren/nime 
├≽️ *#Ninja (teks1/teks2)*
├ _Ex : #Ninja Eren/nime 
